...
export function buildManagementWorkbook(data: ManagementDashboardData, options: { startValue: string; endValue: string; employeeId?: string; processTypeId?: string }) {
  const { startValue, endValue, employeeId, processTypeId } = options;
  ...
  const productions = addSheet(
    ...
    data.rows.map((row) => [
      ...
      row.commissionEarnedAt ? date(row.commissionEarnedAt) : null,
      row.returnReason,
